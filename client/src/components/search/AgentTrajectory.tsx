...
export function parseAgentCompletion(rawText: string): {
  ...
  // Strip control blocks, unclosed control tags, and any trailing unparsed syntax
  ...
  // Replace inline [n] citations with clickable markdown links [[n]](url)
  ...
  return { sources, status, followUps, concepts, markdown };
}
...
export function AgentTrajectory({
  ...
  const { sources, status, followUps, concepts, markdown } = parseAgentCompletion(completion || '');
  const isAgentic = searchMode === 'agentic';

  return (
    <div className="w-full space-y-5">

      {/* Subtle status line while searching (before sources arrive) */}
      ...
      {/* Synthesizing indicator after sources appear */}
      {isAgentic && isAILoading && sources.length > 0 && !markdown && (
        <div className="flex items-center gap-2.5">
          <motion.span
            className="inline-block w-1.5 h-1.5 rounded-full bg-zinc-500"
            animate={{ opacity: [0.3, 1, 0.3] }}
