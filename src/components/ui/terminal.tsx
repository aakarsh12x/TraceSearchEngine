...
export const Terminal = forwardRef<HTMLDivElement, TerminalProps>(({
  ...
  // Use the forwarded ref if provided, otherwise use internal containerRef
  useEffect(() => {
    if (ref) {
  ...
  const content = (
    <div
      ref={containerRef}
      className={cn(
        "border-zinc-800 bg-zinc-950/50 backdrop-blur-xl z-0 w-full rounded-xl border",
        className
      )}
    >
      <div className="border-zinc-800/50 flex flex-col gap-y-2 border-b p-4">
      ...
      <pre className="p-4 max-h-[420px] overflow-y-auto no-scrollbar">
        <code className="grid gap-y-1 overflow-auto whitespace-pre-wrap">{wrappedChildren}</code>
      </pre>
    </div>
  )
